...
from .PythonRobotics.PathPlanning.BezierPath.bezier_path import calc_4points_bezier_path, calc_bezier_path, \
    bernstein_poly, bezier, bezier_derivatives_control_points, curvature, plot_arrow
...
class Kinematics():
    ...
    def vehicleDistortedControlPoints(self, image):
        ...
        for vehicle_color in self.vehicles:
            ...
            aspect_ratio = self.vehicles[vehicle_color]["dimensions"]["car_length_sim"] / \
                           self.vehicles[vehicle_color]["dimensions"]["car_length"]
    ...
    def adjustTrajectoryToSimulation(self):
        ...
        for vehicle_color in self.vehicles:
            ...
            predicted_trajectory = np.hstack([distorted_trajectory[:, 0].reshape(distorted_trajectory.shape[0], 1),
                                              (distorted_height - distorted_trajectory[:, 1]).reshape(
                                                  distorted_trajectory.shape[0], 1)])
        ...
        for vehicle_color in self.vehicles:
            ...
            if (primary_arc_length > 1):
                ...
                if primary_arc_length > ref_arc_length:
                    ...
                    time_stamp = (veh_traj_total_time + diff_arc_len_time) / len(
                        self.vehicles[ref_color]["trajectories"]["distorted_trajectory"])
                    # time_stamp = (veh_traj_total_time) / len(distortory_trajectory)
    ...
    def convertingToScriptFormat(self):
        ...
        script_traj = t1 - t0
        self.time_efficiency["script_traj"] = script_traj + self.time_efficiency["time_stamp_traj"] + \
                                              self.time_efficiency["time_stamp_traj"]
    ...
    def extractKinematicsInformation(self, image_path, vehicles, time_efficiency, output_folder, show_image):
        ...
        t0 = time.time()
        self.vehicles["red"]["trajectories"]["computed"]["bezier_curve"] = self.calBezierPath(vehicle_color="red",
                                                                                              n_points=90)  # 120, 81
        self.vehicles["blue"]["trajectories"]["computed"]["bezier_curve"] = self.calBezierPath(vehicle_color="blue",
                                                                                               n_points=90)  # 120
        t1 = time.time()
        ...
        self.vehicles["red"]["trajectories"]["original_trajectory"] = np.array(
            [waypoint.tolist() for i, waypoint in enumerate(trajectory_red) if (i % 1 == 0)])
        self.vehicles["blue"]["trajectories"]["original_trajectory"] = np.array(
            [waypoint.tolist() for i, waypoint in enumerate(trajectory_blue) if (i % 1 == 0)])
